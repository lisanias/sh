...
$sql_alunos = "SELECT *
        ...
        if ($linhas_alunos = mysqli_num_rows($tabela_alunos)) {
            ...
            while ($dados_alunos = mysqli_fetch_array($tabela_alunos)) {
                ...
                echo "<abbr title='".$dados_alunos['cidade']."/".$dados_alunos['igreja']."'>".$dados_alunos['nome']." ".$dados_alunos['sobrenome']."</ abbr>";
				echo " <a class='icon-remove-sign' href='aluno.acao.php?atp=".base64_encode('tirar_do_quarto')."&id=".base64_encode($dados_alunos['id_matricula'])."' style='color:red' title='Tirar aluno do quarto (esta açao não pode ser desfeita)' ></a><br>";
                $n = $n+1;
