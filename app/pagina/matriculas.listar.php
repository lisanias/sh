...
<?php 
	...
	while ($dados = mysqli_fetch_array($tabela)) {
		echo "<tr><td>".date('d/m/Y', strtotime($dados['data_matricula']))."</td><td><abbr title='".$dados['nome_curso']."'>".$dados['apelido']."</abbr></td><td>".$dados['nome']." ".$dados['sobrenome']."</td><td class='td-actions'>" . fsc($dados['status'])."</td>";
