...
while ($dados = mysqli_fetch_array($tabela)) {
	...
	$txt = 'Declaramos para os devidos fins que '.$artigo.' '.$txaluno.'<stron>';
	$txt = $txt + $dados['nome'].' '.$dados['sobrenome'].'</strong>';
	$txt = $txt + 'participou da jornada modular do '.$dados['nome_curso'];
	...
	$txt = $txt + 'Por ser expressão, da verdade, firmamos a presente declaração e nos colocamos a disposição para maiores esclarecimentos.';
